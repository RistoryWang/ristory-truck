...
func (this *actionService) format(action *entity.Action) {
	...
	case "create_task":
		action.Message = fmt.Sprintf("<b>%s</b> 创建了编号为 <b class='blue'>%d</b> 的工单。", action.Actor, action.ObjectId)
	case "pub_task":
	...
	case "del_task":
		action.Message = fmt.Sprintf("<b>%s</b> 删除了编号为 <b class='blue'>%d</b> 的工单。", action.Actor, action.ObjectId)
	case "review_task":
		task, err := TaskService.GetTask(action.ObjectId)
		if err != nil {
			if action.Extra == "1" {
				action.Message = fmt.Sprintf("<b>%s</b> 审批了编号为 <b class='blue'>%d</b> 的工单，结果为：<b class='green'>通过</b>", action.Actor, action.ObjectId)
			} else {
				action.Message = fmt.Sprintf("<b>%s</b> 审批了编号为 <b class='blue'>%d</b> 的工单，结果为：<b class='red'>不通过</b>", action.Actor, action.ObjectId)
			}
		} else {
			if action.Extra == "1" {
				action.Message = fmt.Sprintf("<b>%s</b> 审批了 <span class='text-primary'>%s</span> 编号为<b>%d</b>的工单，结果为：<b class='green'>通过</b>", action.Actor, task.ProjectInfo.Name, action.ObjectId)
			} else if action.Extra == "-1" {
				action.Message = fmt.Sprintf("<b>%s</b> 审批了 <span class='text-primary'>%s</span> 编号为<b>%d</b>的工单，结果为：<b class='red'>不通过</b>", action.Actor, task.ProjectInfo.Name, action.ObjectId)
			}
		}
	}
}
