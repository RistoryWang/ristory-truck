...
func (this *TaskController) List() {
	...
	//envList, _ := service.EnvService.GetEnvListByProjectId()
	this.Data["pageTitle"] = "工单列表"
...
func (this *TaskController) Create() {
	...
	if this.isPost() {
		...
		//envName := this.GetString("envName"+strconv.Itoa(envId))
//		verType, _ := this.GetInt("ver_type")
		...
		//if typex == "shell" {
		//	endVer = time.Now().Format("20060102150405")
		//}

		message := this.GetString("editor_content")
		if endVer == ""  {
			this.showMsg("请填写发布版本", MSG_ERR)
		}
		if envId < 1 {
			this.showMsg("请选择发布环境", MSG_ERR)
		}
		if message == ""  {
			this.showMsg("请填写版本说明", MSG_ERR)
		}
		//if verType == 2 {
		//	startVer = ""
		//} else {
